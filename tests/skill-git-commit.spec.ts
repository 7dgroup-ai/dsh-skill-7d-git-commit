...
describe('dsh-skill-7d-git-commit', () => {
  it('registers and disposes the bundled git commit skill', async () => {
    ...
    const resourcePath = fileURLToPath(new URL('../assets/git-commit/', import.meta.url)).replace(/[\\/]$/, '')
    ...
    const loaded = await ctx.skills.get('git-commit')
  ...
  it('ships the commit rule reference beside the body', async () => {
    const base = new URL('../assets/git-commit/', import.meta.url)
